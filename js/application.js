...
(function() {
  var CompileProgram, EPSILON, InitBuffers, MobiusTube, NORMAL, POSITION, Render, Slices, Stacks, TEXCOORD, TWOPI, VERTEXID, abs, cos, pow, programs, root, sgn, sin, theta, uniforms, vbos, _ref;
  ...
  CompileProgram = function(vName, fName, attribs, unames) {
    ...
    for (key in unames) {
      value = unames[key];
      uniforms[value] = gl.getUniformLocation(program, key);
    }
    return program;
  };

  root.AppInit = function() {
    var attribs, canvas, gl, h, unames, unif, w;
    canvas = $("canvas");
