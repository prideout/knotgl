...
(function() {
  var assignEventHandlers, collapsing, expanding, exportScreenshot, getPagePosition, layout, root, swipePane, tick, updateNumeralSizes, updateSwipeAnimation;
  ...
  root.AnimateNumerals = function() {
    var collapse, collapsing, duration, expand, expanding;
    if (collapsing || expanding) {
      return;
    }
    duration = 0.25 * root.display.transitionMilliseconds;
    collapse = new TWEEN.Tween(root.numeralSizes).to(metadata.CollapsedSizes, duration).easing(TWEEN.Easing.Quintic.In).onUpdate(updateNumeralSizes).onComplete(function() {
      var collapsing;
      return collapsing = false;
    });
    expand = new TWEEN.Tween(root.numeralSizes).to(metadata.ExpandedSizes, duration).easing(TWEEN.Easing.Quintic.In).onUpdate(updateNumeralSizes).onComplete(function() {
      var expanding;
  ...
  assignEventHandlers = function() {
    ...
    $('#wideband').click(function(e) {
      ...
      if (root.pageIndex === 0 && !(root.swipeTween != null)) {
        if (!(root.display.highlightRow != null)) {
          return;
        }
        root.display.changeSelection(root.display.gallery.i, root.display.highlightRow);
        swipePane();
      }
    });
  ...
  updateNumeralSizes = function() {
    ...
    return $('#subscript').css('font-size', root.numeralSizes.index);
  };
