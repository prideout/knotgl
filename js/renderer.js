...
(function() {
  ...
  Renderer = (function() {
    ...
    Renderer.prototype.compileProgram = function(vName, fName, attribs, uniforms) {
      ...
      numUniforms = this.gl.getProgramParameter(program, this.gl.ACTIVE_UNIFORMS);
      uniforms = (function() {
        var _i, _results;
        _results = [];
        for (u = _i = 0; 0 <= numUniforms ? _i < numUniforms : _i > numUniforms; u = 0 <= numUniforms ? ++_i : --_i) {
          _results.push(this.gl.getActiveUniform(program, u).name);
        }
        return _results;
      }).call(this);
      for (_i = 0, _len = uniforms.length; _i < _len; _i++) {
        u = uniforms[_i];
        program[u] = this.gl.getUniformLocation(program, u);
      }
      return program;
    };
